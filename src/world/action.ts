...
export class MoveAction implements Action {
    ...
    public perform(entity: Entity) {
        ...
        const to = entity.location.getMovedBy(directionVectors[this.direction].x, directionVectors[this.direction].y);
        ...
        if (entity.charSheet.hasSufficientAP(this.cost)) {
...
export class TurnAction implements Action {
    ...
    public perform(entity: Entity) {
        ...
        const inst = Instance.getLoadedInstanceById(entity.location.instance_id);
        if (inst) {
            inst.emit(new TurnEvent(entity, this.direction), entity.location);
        }
...
export class AttackAction implements Action {
    ...
    public perform(entity: Entity) {
        ...
        const attackPos = entity.location.getMovedBy(vec.x, vec.y);
        const opponent = inst.getMobInLocation(attackPos.x, attackPos.y);
        if (opponent) {
            opponent.hit(entity);
        } else {
            inst.emit(new AttackEvent(false, entity, null), entity.location);
        }
        return { 'result': ACTION_RESULT.SUCCESS, 'cost': this.cost };
    }
    public toJSON(): object {
...
export class PickupAction implements Action {
    ...
    public perform(entity: Entity) {
        const inst = Instance.getLoadedInstanceById(entity.location.instance_id)!;
        const pickup = this;
        const inv = entity.charSheet.equipment.inventory;
        ...
        for (let i = 0; i < inv.stacks; i++) {
            ...
            if (this.item_id === stack.item.id) {
                if (this.count === null || stack.count === null || this.count >= stack.count) {
                    inst.dropItem(stack.item, stack.count, entity.location);
                    inv.removeItemFromSlot(i);
                } else {
                    const dropItem = stack.item.clone();
                    stack.count -= this.count;
                    inst.dropItem(dropItem, this.count, entity.location);
                }
                return { 'result': ACTION_RESULT.SUCCESS, 'cost': this.cost };
            }
        }
...
export class EquipAction implements Action {
    ...
    public perform(entity: Entity) {
        const stack = entity.charSheet.equipment.inventory.getItemStackById(this.item_id);
        if (!stack) {
            console.log('Cannot equip nonexistent item!');
            return { 'result': ACTION_RESULT.FAILURE, 'cost': 0 };
        }
        if (stack.item.asWeapon) {
            entity.charSheet.equipment.equipWeapon(entity.charSheet.equipment.inventory.removeItemById(this.item_id)!.asWeapon);
        } else if (stack.item.asArmor) {
            entity.charSheet.equipment.equipArmor(entity.charSheet.equipment.inventory.removeItemById(this.item_id)!.asArmor);
        } else {
...
export class UnequipAction implements Action {
    ...
    public perform(entity: Entity) {
        if (entity.charSheet.equipment.unequip(this.slot)) {
            return { 'result': ACTION_RESULT.SUCCESS, 'cost': this.cost };
        }
        return { 'result': ACTION_RESULT.REDUNDANT, 'cost': 0 };
    }
