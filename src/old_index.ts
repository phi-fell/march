    ...
    app.get('/diagnostic', (req: any, res: any, next: any) => {
        if (validateAdminToken(req.cookies.admin_token)) {
            res.send(
                pug.renderFile(
                    path.resolve(
                        __dirname + '/../site/pug/diagnostic/main.pug',
                    ),
                    {
                        'instances': Instance.accessAllInstances(),
                        'versions': versions,
        ...
        if (validateAdminToken(req.cookies.admin_token)) {
            if (req.query.id) {
                ...
                res.send(
                    pug.renderFile(
                        path.resolve(
                            __dirname + '/../site/pug/diagnostic/instances.pug',
                        ),
                        {
                            'instances': Instance.accessAllInstances(),
                        },
...
app.get('/character_creation', (req: any, res: any) => {
    if (req.cookies.user && req.cookies.auth) {
        validateCredentialsByAuthToken(
            req.cookies.user,
            req.cookies.auth,
            (validate_err: any, valid: boolean) => {
                if (validate_err || !valid) {
                    return res.redirect('/login');
                }
                if (getLoadedUserByName(req.cookies.user)) {
                    return res.send(
                        'You are already logged in on a different window or device.',
                    );
                }
                loadUserByName(
                    req.cookies.user,
                    (load_err: any, user: User) => {
                        if (load_err) {
                        ...
                        return res.send(
                            pug.renderFile(
                                ...
                                {
                                    'essence': 100,
                                    'attributes': Object.keys(ATTRIBUTE).filter(
                                        (attr) =>
                                            typeof ATTRIBUTE[attr as any] ===
                                            'number',
                                    ),
                                    'skills': Object.keys(SKILL).filter(
                                        (attr) =>
                                            typeof SKILL[attr as any] ===
                                            'number',
                                    ),
...
app.post('/character_creation', (req: any, res: any) => {
    if (!req.cookies.user || !req.cookies.auth) {
        res.send({
            ...
            'alert':
                'You are not logged in with valid credentials.  Please log in.  (you can try logging in in a separate window, closing that window, and then continuing from here, so as not to lose your choices)',
        });
        return;
    }
    validateCredentialsByAuthToken(
        ...
        (validate_err: any, valid: boolean) => {
            ...
            if (sheet === null) {
                res.send({
                    ...
                    // tslint:disable-next-line: max-line-length
                    'alert':
                        "You're character is not valid.  This is likely a bug, if so please contact the developer.  (or you messed with the data.  tsk tsk.  nice try)",
                });
                return;
            }
            ...
            loadUserByName(req.cookies.user, (load_err: any, user: User) => {
                if (load_err) {
                    ...
                    res.send({
                        ...
                        // tslint:disable-next-line: max-line-length
                        'alert':
                            'Something went wrong.  User could not be loaded from disk.  This is likely a bug, in which case please contact the developer.',
                    });
                    return;
                }
                if (user.player) {
                ...
                user.player = Player.createPlayer(
                    req.body.name || generateName(),
                    sheet,
                );
